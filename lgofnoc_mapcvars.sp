...
new String: g_sUseConfigDir[MAX_CONFIG_LEN];     // which directory in lgofnoc are we using? none = cfg/

/*
    to do:
        - make it work with lgofnoc (lgofnoc?)
...
public Plugin:myinfo = {
    ...
    author      = "Tabun",
    version     = "0.1b",
    description = "Loads convars on map-load, based on currently active map and lgofnoc config."
};


public OnPluginStart() {
    g_hUseConfigDir = CreateConVar("l4d_mapcvars_configdir", "", "Which lgofnoc config are we using?", FCVAR_PLUGIN);
...
public CvarConfigChange( Handle:cvar, const String:oldValue[], const String:newValue[] ) {
    ...
    #if DEBUG
    PrintToChatAll("[mcv] config directory changed [%s], reloading values.", newValue);
    #endif
...
public GetThisMapPrefs()
{
    ...
    if (!FileExists(usePath)) {
        #if DEBUG
        PrintToChatAll("[mcv] file does not exist! (%s)", usePath);
        #endif
        return 0;
    }
    
    #if DEBUG
    PrintToChatAll("[mcv] trying keyvalue read (from [%s])...", usePath);
    #endif
    ...
    if (hKv == INVALID_HANDLE) {
        #if DEBUG
        PrintToChatAll("[mcv] couldn't read file.");
        #endif
        return 0;
    }
    ...
    if (!KvJumpToKey(hKv, sMapName))
    {
        ...
        CloseHandle(hKv);
        #if DEBUG
        PrintToChatAll("[mcv] couldn't find map (%s)", sMapName);
        #endif
        return 0;
    }
    ...
    if (KvGotoFirstSubKey(hKv, false))                              // false to get values
    {
        do
        {
            ...
            #if DEBUG
            PrintToChatAll("[mcv] kv key found: [%s], reading value...", tmpKey);
            #endif
            ...
            */
            ...
            if (hConVar != INVALID_HANDLE) {
                ...
                #if DEBUG
                PrintToChatAll("[mcv] kv value read: [%s] => [%s])", tmpKey, tmpValueNew);
                #endif
                
                // read, save and set value
                if (!StrEqual(tmpValueNew,"[:none:]")) {
                    GetConVarString(hConVar, tmpValueOld, sizeof(tmpValueOld));
                    PrintToChatAll("[mcv] cvar value changed: [%s] => [%s] (saved old: [%s]))", tmpKey, tmpValueNew, tmpValueOld);
                ...
                #if DEBUG
                PrintToChatAll("[mcv] convar doesn't exist: [%s], not changing it.", tmpKey);
                #endif
            }
...
public ResetMapPrefs()
{
    ...
    #if DEBUG
    PrintToChatAll("[mcv] attempting to reset values, if any...");
    #endif
    ...
    if (KvGotoFirstSubKey(g_hKvOrig, false))                              // false to get values
    {
        do
        {
            ...
            if (StrEqual(tmpKey, "__EOF__")) { 
                #if DEBUG
                PrintToChatAll("[mcv] kv original settings, all read. (EOF).");
                #endif
                break;
            }
            else
            {
                ...
                #if DEBUG
                PrintToChatAll("[mcv] kv original saved setting found: [%s], reading value...", tmpKey);
                #endif
                ...
                if (hConVar != INVALID_HANDLE) {
                    ...
                    #if DEBUG
                    PrintToChatAll("[mcv] kv saved value read: [%s] => [%s])", tmpKey, tmpValueOld);
                    #endif
                    ...
                    if (!StrEqual(tmpValueOld,"[:none:]")) {
                        ...
                        PrintToChatAll("[mcv] cvar value reset to original: [%s] => [%s])", tmpKey, tmpValueOld);
                    }
                } else {
                    #if DEBUG
                    PrintToChatAll("[mcv] convar doesn't exist: [%s], not resetting it.", tmpKey);
                    #endif
                }
            }
